...
- [x] **Lockfile + `npm ci` + lint/typecheck gates in CI** `high · small` — _done (v2.4.2-rc2)._ Committed `package-lock.json`, removed the stale `yarn.lock`, switched all three workflows to `npm ci`, and added `npm run lint` + `npm run typecheck` gates to `build.yml` (so the `no-cond-assign` rule now gates PRs). See **Dependency Hygiene** for the paired dependency work.
...
- [x] **Migrate to scoped Rollup plugins + drop unused devDeps** `med · small` — _done (v2.4.2-rc1)._ `rollup-plugin-{terser,node-resolve,commonjs,babel}` → `@rollup/plugin-*`; removed vestigial `@babel/plugin-proposal-{class-properties,decorators}` (no babel config references them; tsc handles decorators) and unused `check-dependencies`. `@rollup/plugin-node-resolve@13` enforces the package `exports` field, which required changing `lit/decorators` imports to `lit/decorators.js`.
- [x] **Replace `rollup-plugin-typescript2` with `@rollup/plugin-typescript`** `med · small` — _done (v2.4.2-rc2)._ The unmaintained rpt2 (2021) silently stopped transpiling after the dep churn; the official plugin fixed it and cut another ~15 packages.
